 ...
 * href="http://www.xmpp.org/extensions/xep-0199.html">XMPP Ping (XEP-0199)</a>.
 ...
 * still exists.
 * <p>
 * The ping is done at the application level and is therefore protocol agnostic.
 * It will thus work for both standard TCP connections as well as BOSH or any
 * other transport protocol. It will also work regardless of whether the server
 * supports the Ping extension, since an error response to the ping serves the
 * same purpose as a pong.
 * 
 * @author Florian Schmaus
 */
public class KeepAliveManager {
    private static Map<Connection, KeepAliveManager> instances = Collections
            .synchronizedMap(new WeakHashMap<Connection, KeepAliveManager>());
    ...
    /**
     * Retrieves a {@link KeepAliveManager} for the specified {@link Connection}
     ...
     */
    public synchronized static KeepAliveManager getInstanceFor(
            ...
            periodicPingExecutorService = new ScheduledThreadPoolExecutor(1,
                    new ThreadFactory() {
                        ...
                        public Thread newThread(Runnable runnable) {
                            Thread pingThread = new Thread(runnable,
                                    "Smack Keepalive");
     ...
     * <p>
     * <b>Note</b>: Result is -1 if no message has been received since manager
    ...
    private synchronized void schedulePingServerTask() {
        ...
        if (pingInterval > 0) {
            periodicPingTask = periodicPingExecutorService.schedule(
                    new Runnable() {
                        @Override
                        public void run() {
                            Ping ping = new Ping();
                            PacketFilter responseFilter = new PacketIDFilter(
                                    ping.getID());
                            Connection connection = weakRefConnection.get();
                            final PacketCollector response = pingFailedListeners
                                    .isEmpty() ? null : connection
                                    .createPacketCollector(responseFilter);
                            connection.sendPacket(ping);

                            if (response != null) {
                                // Schedule a collector for the ping reply,
                                // notify listeners if none is received.
                                periodicPingExecutorService.schedule(
                                        new Runnable() {
                                            ...
                                            public void run() {
                                                ...
                                                // reply can be ignored since we
                                                // only care if we actually got
                                                // one.
                                                if (result == null) {
